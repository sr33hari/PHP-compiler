%{
	...
	FILE *yyin;
	int yylex();
	char *brlb,*contlb,*swlb,*swfal,*cslb[100],*wltr,*lgtr;
	int i =0;
	char* type;
	char* temp;
	char *var;
...
while
	: T_WHILE T_PAOP {$$.tr = newLabel(&ln);wltr = $$.tr;fprintf(fp,"\n%s:",wltr);}logic_expr T_PACL {$3.tr = newLabel(&ln);$3.fal = newLabel(&ln);$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code, "if T%d go to %s else go to %s\n%s",tn-1,$3.tr,$3.fal,$3.tr);fprintf(fp,"%s",$$.code);brlb = $3.fal;contlb = $$.tr;} block {fprintf(fp,"go to %s\n%s:",wltr,$3.fal);}; 
	| T_WHILE T_PAOP {$$.tr = newLabel(&ln);wltr = $$.tr;fprintf(fp,"\n%s:",wltr);}logic_expr T_PACL {$3.tr = newLabel(&ln);$3.fal = newLabel(&ln);$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code, "if T%d go to %s else go to %s\n%s",tn-1,$3.tr,$3.fal,$3.tr);fprintf(fp,"%s",$$.code);brlb = $3.fal;contlb = $$.tr;} {addNode(list,1);}singlestatement {fprintf(fp,"go to %s\n%s:",wltr,$3.fal);addNode(list,0);}; 
	| T_WHILE T_PAOP {$$.tr = newLabel(&ln);wltr = $$.tr;fprintf(fp,"\n%s:",wltr);}logic_expr T_PACL {$3.tr = newLabel(&ln);$3.fal = newLabel(&ln);$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code, "if %s go to %s else go to %s\n%s",lgtr,$3.tr,$3.fal,$3.tr);fprintf(fp,"%s",$$.code);brlb = $3.fal;contlb = $$.tr;} T_SCLN {fprintf(fp,"go to %s\n%s:",wltr,$3.fal);}; 
;
...
logic_expr
	...
	| rel_expr logic_op logic_expr  {$$.addr = newTemp(&tn); lgtr = strdup($$.addr);$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"%s = %s %s %s\n",$$.addr,$1.addr,$2.code,$3.addr);fprintf(fp,"%s",$$.code);}
;
...
factor
	...
	| variable 							{SYMNODE *new = lookup(var,slist,list);$$.addr = new->val;$$.code = strdup("");if(new == NULL){printf("%s not defined",var);yyerror("");}if(!strcmp(new->type,"int")){$$.type = strdup("int");$$.i = atoi(new->val);}else{$$.type = strdup("float");$$.f = atof(new->val);}}
